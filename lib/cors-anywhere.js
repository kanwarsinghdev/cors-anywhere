...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    maxRedirects: 6,                // Maximum number of redirects to be followed.
    originBlacklist: [],            // Requests from these origins will be blocked.
    originWhitelist: ['http://localhost:9091'],            // If non-empty, requests not from an origin in this list will be blocked.
  ...
  return function(req, res) {
    ...
    console.log('reg ex:', /^\/https?:/.test(req.url));
    console.log(' valid host:', isValidHostName(location.hostname));
    console.log('indexOf:', location.hostname.indexOf("imgprd21.museumofthebible.org"));
    if (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname) && location.hostname.indexOf("imgprd21.museumofthebible.org")== -1) {
